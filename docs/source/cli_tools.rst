...
- ``--zmq-publish-addr`` allows to set the address for the 0MQ 'PUB' socket.

- Setting ``--zmq-publish ON`` enables publishing of the collected output to
  0MQ socket.

- Setting ``--console-output OFF`` disables printing to terminal.

Encryption for Control Connection
+++++++++++++++++++++++++++++++++
...
and used to pass control commands from clients. ``--redis-addr`` is the address of Redis server
used by RE Manager as persistent storage. ``--verbose``, ``--quiet`` and ``--silent`` options
may be used to modify logging verbosity level for RE Manager.
...
.. code-block::
    ...
    usage: start-re-manager [-h] [--zmq-addr ZMQ_ADDR]
                            [--startup-dir STARTUP_DIR | --startup-profile PROFILE_NAME | --startup-module STARTUP_MODULE_NAME | --startup-script STARTUP_SCRIPT_PATH]
                            ...
                            [--zmq-publish-addr ZMQ_PUBLISH_ADDR] [--zmq-publish {ON,OFF}]
    ...
    Configure console output:
      ...
      --zmq-publish-addr ZMQ_PUBLISH_ADDR
...
.. code-block::
    ...
    optional arguments:
      ...
      --address ADDRESS, -a ADDRESS
                            Address of the server (e.g. 'tcp://localhost:60615', quoted string)
