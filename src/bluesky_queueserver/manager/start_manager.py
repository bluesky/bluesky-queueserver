...
def _build_arg_parser_and_parse(s_enc):
    def formatter(prog):
    ...
    parser.add_argument(
        ...
        choices=["ON", "OFF"],
        default="OFF",
        help="Run the manager in Sardana mode, driving a Sardana Pool/Door instead of a bluesky Run Engine. "
        "Requires '--tango_url' (or the 'TANGO_HOST' environment variable) to locate the Tango database, "
        "otherwise the manager fails to start. When 'OFF' (default), '--tango_url' is ignored and the manager "
        "drives a bluesky Run Engine as usual (default: %(default)s).",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Address of the Tango database hosting the Sardana Pool/Door, required when "
            "'--sardana-enable ON' is set. Falls back to the 'TANGO_HOST' environment variable if not given. "
            "Address format: 'localhost:10000' (default: not set)."
        ),
    )
...
def start_manager():
    ...
    if settings.sardana_enable:
        config_worker["tango_url"] = settings.tango_url
        config_manager["tango_url"] = settings.tango_url
