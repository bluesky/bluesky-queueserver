...
#. use logging hooks to publish state
#. harden the json -> plan + object code
#. develop way to describe the allowed plans and devices
#. happi integration
#. ability to add additional plans / devices
#. ability to send a Python module of plans / devices to add
#. re-think communication methods

Server
~~~~~~

#. add logic to server to spawn RE worker with state based on current user

   #. what plans
   #. what devices
   #. where to publish documents to
   #. where to publish RE state to
   #. nanny process to restart etc

#. harden / document protocol between server and
#. add ability to publish the list of plans and devices available to a given user
#. make queue mutable other than addition
#. authentication + session logic
#. business logic to de-conflict usage
#. put the queue in a persistent data structure
